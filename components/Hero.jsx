  ...
  return (
    <header id="top" className="relative overflow-hidden">
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-6 py-24 md:grid-cols-[1.35fr_0.65fr] md:py-28">
        <div>
          <motion.div {...fade(0)} className="mb-7 inline-flex items-center gap-2 rounded-full border border-hair bg-panel/60 px-4 py-2 text-xs text-mist">
            <span className="pulse-dot h-1.5 w-1.5 rounded-full bg-cyan" />
            Available for AI/ML Roles &amp; Internships
          </motion.div>

          <motion.h1 {...fade(0.05)} className="font-display text-5xl font-bold leading-[1.06] tracking-tight text-fog md:text-6xl">
            Building production-ready AI applications with{" "}
            <span className="grad-text">LLMs, RAG &amp; AI Agents</span>
          </motion.h1>

          <motion.p {...fade(0.12)} className="mt-6 max-w-[56ch] text-lg text-mist">
            I build production-ready AI applications using LLMs, Retrieval-Augmented
            Generation (RAG), and AI Agents to deliver accurate, source-backed answers
            for real-world business problems.
          </motion.p>

          {/* trust indicators */}
          <motion.ul {...fade(0.18)} className="mt-7 flex flex-wrap gap-x-6 gap-y-2.5">
            {TRUST.map((t) => (
              <li key={t} className="flex items-center gap-2 text-sm text-fog/90">
                <Check size={15} className="text-cyan" /> {t}
              </li>
            ))}
          </motion.ul>

          <motion.div {...fade(0.24)} className="mt-9 flex flex-wrap gap-3.5">
            <a href="#projects" className="flex items-center gap-2 rounded-full bg-fog px-6 py-3 text-sm font-medium text-night transition hover:-translate-y-0.5 hover:shadow-[0_0_28px_-4px_rgba(255,255,255,0.5)]">
        ...
        {/* right: photo orb (smaller) */}
        <motion.div
          initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.15 }}
        ...
        >
          ...
          <div className="grad-border relative rounded-full p-[2px]">
            <div className="rounded-full bg-night p-2">
              ...
              <img src="/azam.jpg" alt="Azam Hussain" className="aspect-square w-full rounded-full object-cover" />
            </div>
          </div>
        </motion.div>
      </div>
